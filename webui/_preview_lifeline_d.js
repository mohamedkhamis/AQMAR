...
//
// Harness globals used (not reimplemented): esc, computeAge, fmtDate,
...
(function () {
  ...
  var CSS = "\n" +
  /* ---- everything scoped under .v-d so five variants coexist ---- */
  ".v-d { font-family: var(--font-body); }\n" +

  /* headline */
  ".v-d .vd-head { display:flex; align-items:baseline; justify-content:space-between; flex-wrap:wrap; gap:8px 16px; margin-bottom:16px; }\n" +
  ".v-d .vd-kicker { font-family: var(--font-latin-sans); font-size:11px; letter-spacing:0.2em; text-transform:uppercase; color: var(--olive); }\n" +
  ".v-d .vd-lived { font-family: var(--font-display); font-size: clamp(1.15rem, 1rem + 0.9vw, 1.5rem); font-weight:500; margin-top:4px; }\n" +
  ".v-d .vd-lived b { color: var(--forest); font-weight:700; }\n" +
  ".v-d .vd-days { color: var(--muted); font-size:0.8em; }\n" +
  ".v-d .vd-legend { display:flex; gap:14px; flex-wrap:wrap; font-size:11.5px; color: var(--muted); }\n" +
  ".v-d .vd-legend span { display:inline-flex; align-items:center; gap:6px; }\n" +
  ".v-d .vd-sw { flex:none; display:inline-block; }\n" +
  ".v-d .vd-sw-birth { width:9px; height:9px; border-radius:50%; background: var(--olive); }\n" +
  ".v-d .vd-sw-mart { width:9px; height:9px; background: var(--forest); transform: rotate(45deg); }\n" +
  ".v-d .vd-sw-ev { width:9px; height:9px; border-radius:50%; background: var(--paper); border:1.5px solid var(--ink-2); }\n" +
  ...
  ".v-d .vd-lifebar { position:relative; height:10px; margin:14px 9px 26px; border-radius:999px; background: var(--divider); }\n" +
  ".v-d .vd-lifebar-fill { position:absolute; inset:0; border-radius:999px; background: linear-gradient(to right, var(--olive), var(--forest)); }\n" +
  /* linear-gradient has no logical direction: flip so olive stays on the birth
     (right in RTL) side. */
  "html[dir=\"rtl\"] .v-d .vd-lifebar-fill { background: linear-gradient(to left, var(--olive), var(--forest)); }\n" +
  ".v-d .vd-div { position:absolute; top:-3px; bottom:-3px; width:1px; background: var(--bg); transform: translateX(-50%); }\n" +
  ".v-d .vd-ev { position:absolute; top:50%; width:8px; height:8px; border-radius:50%; background: var(--paper); border:1.5px solid var(--ink-2); transform: translate(-50%, -50%); }\n" +
  ".v-d .vd-birth { position:absolute; top:50%; width:13px; height:13px; border-radius:50%; background: var(--olive); box-shadow:0 0 0 2px var(--bg-2); transform: translate(-50%, -50%); }\n" +
  ".v-d .vd-mart { position:absolute; top:50%; width:11px; height:11px; background: var(--forest); box-shadow:0 0 0 2px var(--bg-2); transform: translate(-50%, -50%) rotate(45deg); }\n" +

  /* chapters */
  ".v-d .vd-chapters { display:flex; flex-direction:column; gap:10px; }\n" +
  ".v-d .vd-empty { color: var(--muted); font-size:13px; }\n" +
  ".v-d .vd-chapter { border:1px solid var(--divider); border-radius:12px; background: var(--bg-2); overflow:hidden; }\n" +
  ".v-d .vd-chhead { width:100%; display:flex; align-items:center; gap:12px; text-align:start; background:transparent; border:0; color:inherit; padding:13px 14px; }\n" +
  ".v-d .vd-chhead:hover { background: color-mix(in srgb, var(--forest) 5%, transparent); }\n" + /* faint gold wash on hover, derived from the accent */
  ".v-d .vd-chmain { flex:1; min-width:0; }\n" +
  ".v-d .vd-chtop { display:flex; align-items:baseline; justify-content:space-between; gap:10px; flex-wrap:wrap; }\n" +
  ".v-d .vd-chname { font-family: var(--font-display); font-size:16px; font-weight:600; color: var(--ink); }\n" +
  ".v-d .vd-chcount { font-size:12.5px; color: var(--forest); flex:none; }\n" +
  ".v-d .vd-chrange { font-size:12px; color: var(--muted); margin-top:2px; }\n" +
  /* mini within-chapter strip (decorative, aria-hidden) */
  ".v-d .vd-strip { position:relative; height:12px; margin-top:9px; border-radius:999px; background: color-mix(in srgb, var(--divider) 55%, transparent); }\n" + /* recessed track: divider softened so the dots read as the figure */
  ".v-d .vd-strip-dot { position:absolute; top:50%; width:7px; height:7px; border-radius:50%; background: var(--paper); border:1.5px solid var(--ink-2); transform: translate(-50%, -50%); }\n" +
  ".v-d .vd-chev { flex:none; width:14px; height:14px; color: var(--muted); transform: rotate(0deg); transition: transform .2s ease; }\n" +
  ".v-d .vd-chhead[aria-expanded=\"true\"] .vd-chev { transform: rotate(-180deg); }\n" +

  /* expanded panel — an ordered list, the honest chronological structure */
  ".v-d .vd-panel { padding:2px 14px 12px; }\n" +
  ".v-d .vd-evlist { list-style:none; margin:0; padding:0; }\n" +
  ".v-d .vd-evitem { display:flex; gap:10px; padding-block:9px; border-top:1px solid var(--divider); }\n" +
  ".v-d .vd-evmk { flex:none; width:9px; height:9px; margin-top:6px; border-radius:50%; background: var(--paper); border:1.5px solid var(--ink-2); }\n" +
  ".v-d .vd-evbody { flex:1; min-width:0; }\n" +
  /* names WRAP (no white-space:nowrap) — this is what makes 45-char Arabic
     names safe at 360px. */
  ".v-d .vd-evname { font-size:14px; line-height:1.5; color: var(--ink); overflow-wrap:break-word; }\n" +
  ".v-d .vd-evmeta { margin-top:4px; display:flex; gap:8px; flex-wrap:wrap; align-items:center; }\n" +
  ".v-d .vd-evdate { font-size:12px; color: var(--muted); font-variant-numeric: tabular-nums; }\n" +
  ".v-d .vd-pill { font-size:11.5px; color: var(--forest); background: var(--gold-dim); border:1px solid var(--gold-dim-2); border-radius:999px; padding:1px 9px 2px; }\n" +

  "@media (prefers-reduced-motion: reduce) {\n" +
  "  .v-d .vd-chev { transition: none; }\n" +
  "}\n";
  ...
  function render(person, events, lang) {
    ...
    if (!person || !person.birth || !person.martyrdom) return '<div class="v-d"></div>';
    var t0 = ymdTime(person.birth);
    var tMart = ymdTime(person.martyrdom);
    if (!isFinite(t0) || !isFinite(tMart) || tMart <= t0) return '<div class="v-d"></div>';
    var span = tMart - t0;
        ...
        return '<li class="vd-evitem">' +
                 ...
                 '<span class="vd-evbody">' +
                   ...
                   '<span class="vd-evmeta">' +
                     '<span class="vd-evdate">' + esc(fmtDate(e.start_date, lang)) + "</span>" +
                     agePill +
                   "</span>" +
                 "</span>" +
               "</li>";
  ...
  function mount(rootEl /*, person, events, lang */) {
    if (!rootEl) return;
    var heads = rootEl.querySelectorAll(".v-d .vd-chhead");
