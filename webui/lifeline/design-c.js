// webui/lifeline/design-c.js — "محور العمر" / Age axis
//
...
//
// Styles: this module carries its own CSS, scoped to .lfd-c, and the
...
(function (global) {
  ...
  function D(iso, lang) {
    return typeof formatDate === 'function' ? formatDate(iso, lang) : String(iso || '');
  }
  function tMs(iso) {
    if (!iso) return NaN;
  ...
  // ==========================================================================
  global.AQMAR_LIFELINE_DESIGNS[KEY] = {
    key: KEY,
    labelAr: 'محور العمر',
    ...
    css: [
      /* everything scoped under .lfd-c */
      '.lfd-c, .lfd-c *, .lfd-c *::before, .lfd-c *::after { box-sizing: border-box; }',
      '.lfd-c {',
      ...
      '}',
      '.lfd-c .vc-sr { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0; }',

      /* ---- header ---- */
      '.lfd-c .vc-head { display:flex; align-items:baseline; justify-content:space-between; flex-wrap:wrap; gap:6px 20px; margin-bottom:16px; }',
      '.lfd-c .vc-kicker { font-family: var(--font-latin-sans); font-size:11px; letter-spacing:.2em; text-transform:uppercase; color: var(--olive); }',
      '.lfd-c .vc-lived { font-family: var(--font-display); font-size:22px; font-weight:500; line-height:1.35; color: var(--ink); margin-top:3px; }',
      '.lfd-c .vc-lived b { color: var(--forest); font-weight:600; }',
      '.lfd-c .vc-lived .vc-days { color: var(--muted); font-size:15px; font-family: var(--font-body); }',
      '.lfd-c .vc-count { font-size:12.5px; color: var(--muted); }',

      /* ---- the one grid the axis and every lane share (so x-positions line up) ---- */
      '.lfd-c .vc-row { display:grid; grid-template-columns: var(--vc-label) minmax(0,1fr); column-gap: var(--vc-gap); }',

      /* ---- axis ---- */
      '.lfd-c .vc-axis { align-items:end; margin-bottom:12px; }',
      '.lfd-c .vc-gutter { padding-bottom:6px; }',
      '.lfd-c .vc-scale-note { font-size:11px; line-height:1.55; color: var(--faint); }',
      '.lfd-c .vc-plot { position:relative; height:52px; }',
      '.lfd-c .vc-rail { position:absolute; inset-inline:0; top:0; height:34px; }',
      '.lfd-c .vc-tick { position:absolute; bottom:0; white-space:nowrap; }',
      '.lfd-c .vc-tick-age { display:block; font-size:12px; font-weight:600; font-style:normal; line-height:1.25; color: var(--ink-2); text-align:center; }',
      '.lfd-c .vc-tick-year { display:block; font-family: var(--font-latin-sans); font-size:9.5px; font-style:normal; line-height:1.3; color: var(--faint); font-variant-numeric: tabular-nums; text-align:center; }',
      '.lfd-c .vc-tick::after { content:""; position:absolute; bottom:-6px; left:50%; width:1px; height:6px; background: var(--vc-hair); }',
      '.lfd-c .vc-tick-birth::after, .lfd-c .vc-tick-mart::after { display:none; }',
      '.lfd-c .vc-tick-birth .vc-tick-age, .lfd-c .vc-tick-birth .vc-tick-year { color: var(--olive-2); text-align:start; }',
      '.lfd-c .vc-tick-mart .vc-tick-age, .lfd-c .vc-tick-mart .vc-tick-year { color: var(--forest); text-align:end; }',
      '.lfd-c .vc-track { position:absolute; inset-inline:0; top:40px; height:4px; border-radius:999px; background: linear-gradient(to right, var(--olive), var(--forest)); }',
      /* linear-gradient has no logical direction — flip it so olive is always the birth end */
      'html[dir="rtl"] .lfd-c .vc-track { background: linear-gradient(to left, var(--olive), var(--forest)); }',
      '.lfd-c .vc-mk { position:absolute; transform: translateX(-50%); }',
      '.lfd-c .vc-mk-birth { top:36px; width:12px; height:12px; border-radius:50%; background: var(--olive); box-shadow: 0 0 0 3px var(--paper); }',
      '.lfd-c .vc-mk-mart { top:37px; width:10px; height:10px; background: var(--forest); transform: translateX(-50%) rotate(45deg); box-shadow: 0 0 0 3px var(--paper); }',
      '.lfd-c .vc-axis-dot { position:absolute; top:37px; width:10px; height:10px; border-radius:50%; background: var(--paper); border:2px solid var(--ink-2); box-shadow: 0 0 0 2px var(--paper); transform: translateX(-50%); }',
      '.lfd-c .vc-axis-dot.is-on { border-color: var(--forest); box-shadow: 0 0 0 2px var(--paper), 0 0 0 4px var(--gold-dim-2); }',

      /* ---- lanes ---- */
      '.lfd-c .vc-lanes { list-style:none; margin:0; padding:0; }',
      '.lfd-c .vc-lane { position:relative; align-items:center; min-height:34px; padding-block:5px; padding-inline:6px; margin-inline:-6px; border-radius:7px; }',
      '.lfd-c .vc-lbl { display:flex; align-items:baseline; flex-wrap:wrap; gap:3px 8px; min-width:0; }',
      '.lfd-c .vc-age { flex:none; font-size:11.5px; line-height:1.6; color: var(--forest); background: var(--gold-dim); border:1px solid var(--gold-dim-2); border-radius:999px; padding:0 9px 1px; white-space:nowrap; }',
      '.lfd-c .vc-name { font-size:13.5px; font-weight:600; line-height:1.45; color: var(--ink); min-width:0; }',
      '.lfd-c .vc-date-inline { font-size:11.5px; line-height:1.5; color: var(--muted); font-variant-numeric: tabular-nums; white-space:nowrap; }',
      '.lfd-c .vc-cell { position:relative; align-self:stretch; min-height:22px; }',
      '.lfd-c .vc-stem { position:absolute; bottom:50%; width:1px; height: var(--vc-stem, 40px); background: var(--vc-stem-color); transform: translateX(-50%); }',
      '.lfd-c .vc-bar { position:absolute; top:50%; height:2px; border-radius:999px; transform: translateY(-50%); opacity:.5;',
      ...
      'html[dir="rtl"] .lfd-c .vc-bar { background-image: linear-gradient(to left, var(--olive), var(--forest)); background-position: right center; }',
      '.lfd-c .vc-dot { position:absolute; top:50%; width:9px; height:9px; border-radius:50%; background: var(--paper); border:2px solid var(--ink-2); transform: translate(-50%,-50%); }',
      ...
      '.lfd-c .vc-date { position:absolute; bottom:50%; margin-bottom:3px; font-size:11px; line-height:1.35; color: var(--muted); font-variant-numeric: tabular-nums; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }',
      '.lfd-c .vc-date { display:none; }',
      '.lfd-c .vc-lane.has-plot-date .vc-date { display:block; }',
      '.lfd-c .vc-lane.has-plot-date .vc-date-inline { display:none; }',
      '.lfd-c .vc-empty { font-size:13px; color: var(--muted); padding-block:8px; }',

      /* ---- hover: link a lane to its dot on the axis ---- */
      '.lfd-c .vc-lane:hover { background: var(--vc-hover); }',
      '.lfd-c .vc-lane:hover .vc-bar { opacity:1; }',
      '.lfd-c .vc-lane:hover .vc-dot { border-color: var(--forest); }',
      '.lfd-c .vc-lane:hover .vc-stem { background: color-mix(in srgb, var(--forest) 55%, transparent); }',
      '.lfd-c .vc-lane:hover .vc-date { color: var(--ink-2); }',
      /* transitions only exist when motion is welcome — no override needed */
      '@media (prefers-reduced-motion: no-preference) {',
      '  .lfd-c .vc-lane, .lfd-c .vc-bar, .lfd-c .vc-dot, .lfd-c .vc-stem, .lfd-c .vc-date, .lfd-c .vc-axis-dot { transition: background-color .16s ease, background .16s ease, border-color .16s ease, opacity .16s ease, box-shadow .16s ease, color .16s ease; }',
      '}',
      ...
      '  .lfd-c .vc-row { grid-template-columns: minmax(0,1fr); row-gap:7px; }',
      '  .lfd-c .vc-axis { align-items:start; }',
      '  .lfd-c .vc-gutter { padding-bottom:10px; }',
      '  .lfd-c .vc-lane { align-items:start; min-height:0; padding-block:9px; border-radius:0; }',
      '  .lfd-c .vc-lane + .vc-lane { border-top:1px solid var(--vc-hair); }',
      '  .lfd-c .vc-cell { align-self:auto; min-height:0; height:12px; }',
      '  .lfd-c .vc-stem { display:none; }',
      '  .lfd-c .vc-lane.has-plot-date .vc-date { display:none; }',
      '  .lfd-c .vc-lane.has-plot-date .vc-date-inline { display:inline; }',
      '  .lfd-c .vc-lived { font-size:19px; }',
      '}'
    ].join('\n'),
