// webui/lifeline/design-b.js — "العمود الزمني" / Vertical spine
//
...
//
// Styles: this module carries its own CSS, scoped to .lfd-b, and the
...
(function (global) {
  ...
  // ---- harness globals (esc / formatDate / toArDigits / eventsForPerson) with
  ...
  function _date(iso, lang) {
    if (typeof global.formatDate === "function") return global.formatDate(iso, lang);
...
.lfd-b[data-lang="en"] { font-family: var(--font-latin-sans); }
...
.lfd-b .vb-count { font-size: 13px; color: var(--muted); margin-top: 5px; }

.lfd-b .vb-modes { display: inline-flex; gap: 6px; }
.lfd-b .vb-mode {
...
.lfd-b .vb-mode:hover { color: var(--ink-2); border-color: var(--olive); }
.lfd-b .vb-mode[aria-pressed="true"] {
  color: var(--forest); background: var(--gold-dim); border-color: var(--gold-dim-2);
}
.lfd-b .vb-mode:focus-visible { outline: 2px solid var(--olive-2); outline-offset: 2px; }
@media (prefers-reduced-motion: reduce) {
  .lfd-b .vb-mode { transition: none; }
}

.lfd-b .vb-list { list-style: none; margin: 0; padding: 0; }
...
.lfd-b .vb-row.is-birth .vb-rail { top: calc(var(--gp) + var(--vb-mc)); }
.lfd-b .vb-row.is-mart  .vb-rail { bottom: auto; height: calc(var(--gp) + var(--vb-mc)); }
...
.lfd-b .vb-body { margin-inline-start: calc(var(--vb-age-w) + 12px); min-width: 0; }
.lfd-b .vb-name {
...
.lfd-b .vb-row.is-birth .vb-name,
.lfd-b .vb-row.is-mart  .vb-name { font-family: var(--font-display); font-size: 15px; }
.lfd-b .vb-row.is-birth .vb-name { color: var(--olive-2); }
.lfd-b .vb-row.is-mart  .vb-name { color: var(--forest); }
.lfd-b .vb-meta {
...
.lfd-b .vb-days { font-size: 11.5px; color: var(--faint); font-variant-numeric: tabular-nums; }
...
.lfd-b .vb-pill .u { font-size: 10px; opacity: .82; margin-inline-start: 3px; }
.lfd-b .vb-row.is-mart .vb-pill {
...
.lfd-b.is-compact .vb-row.has-gap { --gp: 14px; }
.lfd-b.is-compact .vb-span { display: none; }
...
@media (max-width: 560px) {
  .lfd-b { --vb-gs: .6; --vb-ind: 26px; }
  .lfd-b .vb-body { margin-inline-start: 0; }
  .lfd-b .vb-age { position: static; width: auto; }
  .lfd-b .vb-name { font-size: 13.5px; }
  .lfd-b .vb-row.is-birth .vb-name,
  .lfd-b .vb-row.is-mart  .vb-name { font-size: 14.5px; }
  .lfd-b .vb-head { margin-bottom: 14px; }
}
...
`;
  ...
  function render(person, events, lang) {
    var ar = lang !== "en";
    if (!person || !person.birth || !person.martyrdom) return '<div class="lfd-b"></div>';
    var tB = tOf(person.birth), tM = tOf(person.martyrdom);
    if (!isFinite(tB) || !isFinite(tM) || tM <= tB) return '<div class="lfd-b"></div>';
    ...
    var h = '<div class="lfd-b" data-lang="' + (ar ? "ar" : "en") + '">';
  ...
  function mount(rootEl) {
    if (!rootEl) return;
    var root = (rootEl.classList && rootEl.classList.contains("lfd-b"))
      ? rootEl : rootEl.querySelector(".lfd-b");
