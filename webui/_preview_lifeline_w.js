...
window.VARIANTS['w'] = {
  ...
  css: `
.v-w .lifeline-head { display: flex; align-items: baseline; justify-content: space-between; flex-wrap: wrap; gap: 10px; margin-bottom: 22px; }
.v-w .lifeline-kicker { font-family: var(--font-latin-sans); font-size: 11px; letter-spacing: 0.2em; color: var(--olive); text-transform: uppercase; }
.v-w .lifeline-lived { font-family: var(--font-display); font-size: 22px; font-weight: 500; margin-top: 4px; }
.v-w .lifeline-lived b { color: var(--forest); }
.v-w .lifeline-lived .days { color: var(--muted); font-size: 16px; }
.v-w .lifeline-legend { display: flex; gap: 18px; font-size: 12px; color: var(--muted); flex-wrap: wrap; align-items: center; }
.v-w .lifeline-legend > span { display: inline-flex; align-items: center; gap: 6px; }
.v-w .sw { flex: none; display: inline-block; }
.v-w .sw-birth { width: 10px; height: 10px; border-radius: 50%; background: var(--olive); box-shadow: 0 0 0 3px var(--paper), 0 0 0 4px var(--olive); }
.v-w .sw-mart { width: 10px; height: 10px; background: var(--forest); transform: rotate(45deg); }
.v-w .sw-event { width: 10px; height: 10px; border-radius: 50%; background: var(--paper); border: 2px solid var(--ink-2); }
...
.v-w .lifeline { position: relative; margin-inline: 12px; --axis: 140px; }
.v-w .lifeline-leaders { position: absolute; inset: 0; width: 100%; height: 100%; overflow: visible; pointer-events: none; }
.v-w .lifeline-leaders line { stroke: var(--faint); stroke-width: 1; }
.v-w .lifeline-base { position: absolute; inset-inline: 0; top: var(--axis); height: 2px; background: var(--divider); }
.v-w .lifeline-life { position: absolute; top: calc(var(--axis) - 2px); height: 6px; border-radius: 999px; background: linear-gradient(to right, var(--olive), var(--forest)); }
html[dir="rtl"] .v-w .lifeline-life { background: linear-gradient(to left, var(--olive), var(--forest)); }
.v-w .lifeline-tick { position: absolute; top: calc(var(--axis) - 3px); width: 1px; height: 8px; background: var(--faint); transform: translateX(-50%); }
.v-w .lifeline-tick span { position: absolute; top: 11px; left: 50%; transform: translateX(-50%); font-family: var(--font-latin-sans); font-size: 10px; color: var(--muted); font-variant-numeric: tabular-nums; }
.v-w .lifeline .mk { position: absolute; transform: translateX(-50%); }
.v-w .lifeline .mk-birth { top: calc(var(--axis) - 8px); width: 16px; height: 16px; border-radius: 50%; background: var(--olive); border: 3px solid var(--paper); box-shadow: 0 0 0 1px var(--olive); }
.v-w .lifeline .mk-mart { top: calc(var(--axis) - 7px); width: 14px; height: 14px; background: var(--forest); box-shadow: 0 0 0 3px var(--paper), 0 0 0 4px var(--forest); transform: translateX(-50%) rotate(45deg); }
.v-w .lifeline .mk-event { top: calc(var(--axis) - 6px); width: 12px; height: 12px; border-radius: 50%; background: var(--paper); border: 2px solid var(--ink-2); box-shadow: 0 0 0 2px var(--paper); }
.v-w .mk-year { position: absolute; transform: translateX(-50%); white-space: nowrap; font-family: var(--font-naskh); font-size: 12px; font-weight: 700; }
.v-w .mk-year-birth { top: calc(var(--axis) - 32px); color: var(--olive); }
.v-w .mk-year-mart { top: calc(var(--axis) + 16px); color: var(--forest); }

/* THE CHANGE: wrap within a capped width instead of white-space:nowrap. */
.v-w .lifeline .ev-label { position: absolute; text-align: center; white-space: normal;
  width: 108px; line-height: 1.25; visibility: hidden; overflow-wrap: break-word; }
.v-w .lifeline .ev-label .n { font-size: 12px; font-weight: 600; color: var(--ink-2); }
.v-w .lifeline .ev-label .a { font-size: 10.5px; color: var(--forest); margin-top: 1px; }
/* mount() flags anything it could not place honestly (edge clamp > 1 year). */
.v-w .lifeline .ev-label.drifted { outline: 1px solid var(--crimson); border-radius: 4px; }

.v-w .rows-note { margin-top: 14px; font-size: 12px; color: var(--faint);
  font-family: var(--font-latin-sans); }

/* ≤480px the current design already swaps to the vertical list; unchanged. */
.v-w .below480-note { display: none; color: var(--muted); font-size: 13px; padding: 8px 0; }
@media (max-width: 480px) {
  .v-w .lifeline, .v-w .rows-note { display: none; }
  .v-w .below480-note { display: block; }
}
`,
  ...
  render(m, allEvents, lang) {
    ...
    evs.forEach((e) => {
      ...
      const tip = `${full} — ${fmtDate(e.start_date, lang)}`;
  ...
  mount(root) {
    ...
    const note = root.querySelector('.rows-note');
    if (note) {
      const drifted = items.filter(i => i.driftYears > 1).length;
      note.textContent = `${items.length} events · ${rowCount} row(s) · line ${height}px`
        + (drifted ? ` · ${drifted} edge-clamped` : ' · every label exactly over its date');
    }
  },
};
